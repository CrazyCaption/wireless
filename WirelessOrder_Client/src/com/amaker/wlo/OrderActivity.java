...
//		Cursor c = managedQuery(uri, projection, null, null, null);
		/*
		 * 返回一个ContentResolver实例为您的应用程序的包
		 * public final Cursor query(Uri uri,String[] projection,
		 *                           String selection,String[] selectionArgs,String sortOrder)
		 * uri uri,使用内容:/ /计划,内容检索。　　
		 * projection——哪些列返回的列表。传递null将返回所有列,这是气馁,以防止读取数据存储,不会被使用。　　
		 * selection——一个过滤器声明哪些行返回,格式化为一个SQL WHERE子句(不包括自身)的地方。传递null将返回给定URI的所有行。　　
		 * selectionArgs——你可能包括?在选择,从selectionArgs值所取代,在它们出现的顺序选择。将被绑定的值作为字符串。　　
		 * sortOrder——如何命令行,格式化为一个SQL order BY子句(不包括order BY本身)。传递null将使用默认的排序顺序,这可能是无序的。
		 * Cursor这个接口提供随机读写访问一个数据库查询返回的结果集
		 */
		Cursor c = getContentResolver().query(uri, projection, null, null, null);
		// 实例化桌号下拉列表Spinner的Adapter
		/*
		 * 一个简单的适配器映射列从光标到textview或imageview在XML文件中定义。
		 * 你可以指定哪些列,这观点你想要显示的列,和XML文件定义了这些视图的外观。
		 * 结合发生在两个阶段。首先,如果一个SimpleCursorAdapter。
		 * ViewBinder可用,SimpleCursorAdapter.ViewBinder.setViewValue(android.view。
		 * 视图,android.database。光标,int)被调用。
		 * 如果返回值为真,绑定已经发生。如果返回值为假,
		 * 视图绑定是一个TextView setViewText调用(TextView,字符串)。
		 * 如果返回值为假,视图绑定是一个ImageView,setViewImage调用(ImageView,字符串)。
		 * 如果没有可以找到合适的绑定,IllegalStateException抛出。如果使用这个适配器与过滤,
		 * 例如在一个AutoCompleteTextView,您可以使用SimpleCursorAdapte
		 */
