...
//				// 解析XML文件获得菜单id
				...
				String remark = doc.getElementsByTagName("remark").item(i)
						.getFirstChild().getNodeValue();
				System.out.println(id+","+typeId+","+name);
				// 添加到ContenValues对象
				values.put("_id", id);
	...
	//更新是清空数据
	private void onDeleteMEN(){
		SQLiteDatabase db = openOrCreateDatabase(DBHelper.DATABASE_NAME,
				Context.MODE_PRIVATE, null);
		db.execSQL("DELETE FROM MenuTbl");
		db.close();
	}
	private void onDeleteTAB(){
		SQLiteDatabase db = openOrCreateDatabase(DBHelper.DATABASE_NAME,
				Context.MODE_PRIVATE, null);
		db.execSQL("DELETE FROM TableTbl");
		db.close();
	}
}
